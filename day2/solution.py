...
def find_minimum_viable_number(one_game_stats):
    rounds = len(one_game_stats)
    mvn_red = max([one_game_stats[i][0] for i in range(rounds)]) 
    mvn_green = max([one_game_stats[i][1] for i in range(rounds)]) 
    mvn_blue = max([one_game_stats[i][2] for i in range(rounds)]) 
    return (mvn_red, mvn_green, mvn_blue)


def compute_cube_power(one_game_stats):
    return reduce(mul, find_minimum_viable_number(one_game_stats), 1)
...
def main():
    ...
    for id, color_freqs in game_stats.items():
        if is_game_valid(color_freqs, 12, 13, 14):
            id_sum += int(id)
        power_sum += compute_cube_power(color_freqs)
    print("ID Sum = {}, Cube Power = {}".format(id_sum, power_sum))
